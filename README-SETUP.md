...
- Bài mới đăng ký sẽ có trạng thái **"Chờ duyệt"** và **không hiện công khai** cho tới khi BTC vào khu vực **Quản trị** (nút góc trên bên phải) đổi sang **"Đã duyệt"**. Cũng có thể **tick chọn nhiều bài rồi xóa hàng loạt** bằng nút "Xóa các bài đã chọn".
- Học viên bắt buộc phải tick ô **"Chấp nhận cho Suleco sử dụng hình ảnh cho mục đích truyền thông"** mới đăng ký được; nội dung đồng ý này được lưu vào cột `DongYSuDungAnh` trong Sheet để đối chiếu sau này. Nút "Gửi đăng ký" sẽ **bị khóa (mờ đi)** cho tới khi điền đủ mọi trường bắt buộc (Họ tên, 2 ảnh, Link Facebook, tick đồng ý).
- Cột **Timestamp** được ghi theo giờ Việt Nam (Asia/Ho_Chi_Minh) cố định trong code, không phụ thuộc cài đặt múi giờ của Google Sheet — nếu vẫn thấy sai giờ ở các dòng cũ (đăng ký trước khi cập nhật), đó là do dữ liệu cũ, các lượt đăng ký mới sẽ đúng.
...
- **Không tự kéo đổi vị trí cột trong Google Sheet** (ví dụ kéo cột `SoComment` lên cạnh `SoLike`) — dù code hiện đã tìm cột theo tên (an toàn hơn), việc đổi vị trí cột thủ công không có lợi ích gì và dễ gây nhầm lẫn khi đọc dữ liệu bằng mắt. Nếu muốn đổi thứ tự hiển thị, báo lại để mình sửa trong code cho đồng bộ.
